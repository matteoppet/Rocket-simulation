...
class StageGroup:
    def __init__(self, name, group_components, motor):
        self.name = name
...
class Component(pygame.sprite.Sprite):
    ...
    def update(self, physics: classmethod, time_step: int, colliding: bool, rocket):
        ...
        if self.is_attached:
            ...
            if self.parent:
                ...
                components = []
                for stage in STAGES:
                    components.append(*stage.group_components.sprites())        
    ...
    def update_position(self, physics, group, time_step, colliding, motor_group):
        thrust = 0
        for motor in reversed(list(motor_group)): # TODO: CHANGE by just naming the motor differently and assing each to the stage group 
            if motor.active:
...
class Rocket:
    def __init__(self, components, environment):
        ...
        parent = None
        for stage, components in self.components.items():
            sprite_stage_group = pygame.sprite.Group()
            motor = None

            for component, attribs in components.items():
                if component == "motor":
                    motor = Motor(component, parent, attribs, [sprite_stage_group, self.motor_sprite_group])
                else:
                    parent = Component(component, parent, attribs, sprite_stage_group)
            
            stage_group_instance_class = StageGroup(stage, sprite_stage_group, motor)
            STAGES.append(stage_group_instance_class)
    ...
    def render(self, screen):
        for group in STAGES:
            for sprite in group.group_components:
    ...
    def update(self, time_step, other_sprites: pygame.sprite.Group):
        for group in STAGES:
            colliding = self.collision(group.group_components, other_sprites)
            group.group_components.update(self.physics, time_step, colliding, STAGES)
    ...
    def controls(self):
        ...
        if keys[pygame.K_l]:
            self.detatch_stage_logic()

    def detatch_stage_logic(self):
